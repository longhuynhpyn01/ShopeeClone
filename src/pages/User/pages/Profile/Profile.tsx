...
// Tách thành component riêng để sử dụng useFormContext
// Chú ý cần khai báo FormProvider bọc bên ngoài thì mới sử dụng được
function Info() {
  const {
    register,
    control,
...
}; // Trick để đổi kiểu date_of_birth từ date sang string bởi vì message lỗi trả về luôn là string
...
export default function Profile() {
  ...
  // Lưu trữ file image khi chọn trong máy tính
  const [file, setFile] = useState<File>();

  // Để tạo ra bản preview avatar để xem trước
  ...
  // Lấy từ form dùng watch, còn getValues chỉ nên dùng khi xử lí trong function khi trigger gì đó
  const avatar = watch("avatar"); // render lại khi avatar thay đổi
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      // Xử lí upload avatar lên server để nhận được avatarName
      if (file) {
      ...
      refetch(); // refresh lại cập nhật thay đổi
        ...
        <form className="flex flex-col-reverse mt-8 md:flex-row md:items-start" onSubmit={onSubmit}>
          <div className="flex-grow mt-6 md:mt-0 md:pr-12">
            ...
            <Controller
              ...
              name="date_of_birth"
              render={({ field }) => (
                // không thể truyền props {...field} vì nó sẽ warning ref nên ngoài việc dùng forwardRef thì ta truyền
                // value và onChange bằng thủ công
                <DateSelect
